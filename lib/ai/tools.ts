...
// Factory function to create tools with a HevyClient instance
export function createHevyTools(client: HevyClient) {
  ...
  const getRecentWorkoutsTool = {
    ...
    execute: async ({ count }: { count: number }) => {
      const response = await client.getWorkouts(1, count)
      return {
  ...
  const getWorkoutStatsTool = {
    description:
      "Get overall workout statistics including total count and workout frequency. Use this for high-level training insights.",
    parameters: z.object({}),
    execute: async () => {
      const countResponse = await client.getWorkoutsCount()
      const recentWorkouts = await client.getWorkouts(1, 30)
  ...
  const getRoutinesTool = {
    ...
    execute: async ({ count }: { count: number }) => {
      const response = await client.getRoutines(1, count)
      return {
  ...
  const analyzeTrainingPatternsTool = {
    description:
    ...
    execute: async ({ days }: { days: number }) => {
      ...
      const response = await client.getWorkouts(1, pageSize)
  ...
  const createRoutineTool = {
    ...
    execute: async ({ title, exercises, notes, folder_id }: {
      ...
      const routine = await client.createRoutine(routinePayload)
  ...
  const updateRoutineTool = {
    ...
    execute: async ({ routine_id, title, exercises, notes }: {
      ...
      const existingRoutine = await client.getRoutine(routine_id)
      ...
      const updatedRoutine = await client.updateRoutine(routine_id, updateData)
